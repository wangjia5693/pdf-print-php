...
#浏览器引擎,html5+css3的解析器，并且是完整支持css3 @page部分的打印特性的功能。并且具备生成PDF的功能
...
class Prince
{
    ...
    /**
     * 添加一个CSS样式表将被应用到每个输入文档。
     * 可以调用不止一次添加多个样式表。可以调用转换函数调用之前这个函数。
    ...
    */
    ...
    /**
     * 将多个XML or HTML 文件转化为 PDF 文件.并将转化后的内容输出给php outputbuffer
     * 与covert_file功能一致，不过多了一个输出名自定义
     * 需要申明消息头
    ...
    /**
     * XML or HTML 文件转化为 PDF 文件.并将转化后的内容输出给php outputbuffer
     * 与covert_file功能一致，不过多了一个输出名自定义
     * 需要申明消息头
     * header('Content-Type: application/pdf');
     * header('Content-Disposition: inline; filename="foo.pdf"');
    ...
    /**
     * XML or HTML 字符串转化为 PDF 文件.并将转化后的内容输出给php outputbuffer
     * 与covert_file功能一致，不过多了一个输出名自定义；
     * 需要申明消息头
     * header('Content-Type: application/pdf');
     * header('Content-Disposition: inline; filename="foo.pdf"');
